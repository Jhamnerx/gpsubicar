function MapTiles() {
        ...
        _maps = {
            ...
            10: {
                ...
                tile: function () {
                    ...
                    var tilesLink = 'https://maps.hereapi.com/v3/{resource}/{projection}/{z}/{x}/{y}/{format}?apiKey={apiKey}&style={style}&size={size}&lang={lang}';

                    if (app.settings.keys.here_political_view) {
                        tilesLink += '&pview=' + app.settings.keys.here_political_view;
                    }

                    var _tileLayer = L.tileLayer(tilesLink, {
                        attribution: 'Map &copy; 2025 <a href="http://developer.here.com">HERE</a>',
                        resource: 'base',
                        projection: 'mc',
            ...
            28: {
                ...
                traffic: true,
                tile: function () {
                    if (!app.settings.keys.azure_maps_key)
                        return false;

                    var _tileLayer = L.tileLayer("https://atlas.microsoft.com/map/tile?subscription-key={subscriptionKey}&api-version={apiVersion}&tilesetId={tilesetId}&zoom={z}&x={x}&y={y}&tileSize={tileSize}&language={language}&view={view}", {
                        tilesetId: 'microsoft.base.road',
                        subscriptionKey: app.settings.keys.azure_maps_key,
                        view: 'Auto',
                        language: app.lang.locale.replace('_', '-'),
                        apiVersion: '2.0',
                        tileSize: 256,
                        minZoom: 1,
                        maxZoom: 21,
                        attribution: '<a href="https://azure.microsoft.com" target="_blank">© TomTom, Microsoft</a>',
                    });

                    _tileLayer.overlayLayer = function () {
                        let _overlayLayers = [];

                        if (app.settings.showTraffic) {
                            _overlayLayers.push(L.tileLayer("https://atlas.microsoft.com/map/tile?subscription-key={subscriptionKey}&api-version={apiVersion}&tilesetId={tilesetId}&zoom={z}&x={x}&y={y}&tileSize={tileSize}&language={language}&view={view}", {
            ...
            29: {
                ...
                tile: function () {
                    ...
                    _tileLayer.overlayLayer = function () {
                        let _overlayLayers = [L.tileLayer("https://atlas.microsoft.com/map/tile?subscription-key={subscriptionKey}&api-version={apiVersion}&tilesetId={tilesetId}&zoom={z}&x={x}&y={y}&tileSize={tileSize}&language={language}&view={view}", {
                    ...
                    _tileLayer.map_id = 29;

                    return _tileLayer;
                },
            },
            30: {
                name: 'Azure Satellite',
                maxZoom: 19,
                traffic: true,
                tile: function () {
                    if (!app.settings.keys.azure_maps_key)
            ...
            96: {
                ...
                tile: function() {
                    ...
                    _tileLayer.map_id = 96;

                    return _tileLayer;
                }
            },
            97: {
                name: 'Custom Overlays Map',
                maxZoom: 18,
                tile: function() {
                    var _tileLayer = new L.TileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {maxZoom: 18});
                    var _settings = {};
                    ...
                    $.ajax({
                        url: window.location.origin + '/assets/custom/map-overlays/settings.json',
                        async: false,
                        dataType: 'json',
                        success: function (data) {
                            ...
                            if (data.bounds instanceof Array && data.bounds.length === 4) {
                                _settings.bounds = new L.LatLngBounds(
                                    new L.LatLng(data.bounds[0], data.bounds[1]),
                                    new L.LatLng(data.bounds[2], data.bounds[3])
                                );
                            }

                            if (data.minZoom)
                                _settings.minZoom = data.minZoom;

                            if (data.maxZoom)
                                _settings.maxZoom = data.maxZoom;
                        },
                    });

                    if (_overlay) {
                        _tileLayer.overlayLayer = L.tileLayer('assets/custom/map-overlays/{z}/{x}/{y}.png', _settings);
                    }

                    _tileLayer.map_id = 97;
    ...
    _this.initControls = function() {
        ...
        $(layersControl.onAdd(_this.map)).insertBefore($('#map-controls-tile > div').first());
    };
    ...
    _this.addOverlayLayerEvents = function (_tileLayer) {
      ...
      _tileLayer.on('add', function (layer) {
        overlayLayers = typeof layer.target.overlayLayer === 'function'
            ? layer.target.overlayLayer()
            : layer.target.overlayLayer;

        if (!(overlayLayers instanceof Array)) {
            overlayLayers = [overlayLayers];
        }

        $.each(overlayLayers, function (index, overlay) {
            if (_this.map.hasLayer(overlay)) {
                return;
            }

            overlay.setZIndex(layer.target.zIndex)
    ...
    _this.init = function (map) {
        ...
        _this.aircraft = new Aircraft();
        _this.aircraft.setMap(_this.map);

        if (app.settings.showAircraft) {
            _this.aircraft.show();
        }

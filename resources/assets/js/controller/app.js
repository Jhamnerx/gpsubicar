var app = {
    ...
    checkPageLoad: function(items, timeToSet, id) {
        if ( items != null && items.data ) {
            app.devices.addMulti(items.data);
        ...
        if (!items.pagination.next_page_url) {
            app.checkComplete(timeToSet, id);
        ...
        $.ajax({
            type: 'GET',
            dataType: "json",
            url: items.pagination.next_page_url,
            success: function(res){
                app.checkPageLoad(res.items, timeToSet);
            },
            error: function(jqXHR, textStatus, errorThrown) {
                app.checkComplete(timeToSet, id);
    ...
    checkComplete: function(timeToSet, id) {
        app.checkTimestamp = timeToSet;
    ...
    check: function(id) {
        $.ajax({
            ...
            data: {
                ...
                filters: app.devices.getFiltersMap()
            },
            success: function(res){
                ...
                app.checkPageLoad(res.items, res.time, id);
            },
            complete: function(){},
            error: function(jqXHR, textStatus, errorThrown) {
                handlerFail(jqXHR, textStatus, errorThrown);
    ...
    changeSetting: function(setting, value){
        ...
        switch (setting) {
            ...
            case 'showAircraft':
                ...
                this.settings.showAircraft
                    ? this.maps.aircraft.show()
                    : this.maps.aircraft.hide();
    ...
    saveSetting: function(setting, value) {
        ...
        switch (setting) {
            ...
            case 'clusterDevice':
                var _values = {
                    "clusterDevice":"m_objects_cluster",
                    ...
                    "showTraffic":"m_traffic",
                    "showAircraft":"m_aircraft",
...
function bottomAutoWidth(timeout) {
    setTimeout(function(){
        var _left = 0,
            _height = 0,
            _zindex = 997,
        ...
        $('.map-controls.top-left').css('padding-left', ($sidebar.is(':visible') ? $sidebar.width() : 0) + 'px');
        $('.map-controls.bottom-right').css('padding-bottom', ($bottombar.is(':visible') ? $bottombar.height() : 0) + 'px')
    }, timeout ? 460 : 100);
}

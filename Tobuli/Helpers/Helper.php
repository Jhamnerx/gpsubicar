...
{
    return CustomFacades\GeoLocation::resolveAddressHandled($lat, $lon);
}

function prepareServiceData(array $input, array $values, ?User $user = null): array
{
    ...
    if ($input['expiration_by'] == 'days') {
        ...
        if ($user) {
            $last_service = $last_service->format('Y-m-d H:i:s');
            $last_service = Formatter::time()->reverse($last_service);
            $last_service = Carbon::parse($last_service);
        }
        ...
        if ($today >= $input['expires_date'] && !empty($input['renew_after_expiration'])) {
        ...
        $input['expires_date'] = $input['expires_date']->format('Y-m-d H:i:s');
        $input['remind_date'] = $input['remind_date']->format('Y-m-d H:i:s');
        $input['last_service'] = $input['last_service']->format('Y-m-d H:i:s');
    } else {
        ...
        if ($value >= $input['expires'] && !empty($input['renew_after_expiration'])) {
...
function getMaps()
{
    ...
    if (config('addon.custom_overlays_map')) {
        $maps['Custom Overlays Map'] = 97;
    }
...
function tooltipMarkImei($img, $text)
{
    ...
    return tooltipMark('<span class="text">' . htmlentities($text, ENT_QUOTES) . '</span>', $options);
}
...
function runCacheEntity($entityClass, $ids): \Illuminate\Database\Eloquent\Collection
{
...
function getSensorFuelDifference(DeviceSensor $sensor, $positions, $default = null): array
{
    ...
    $result = [
        'percent'       =>  empty($bigger) ? 0 : ($difference / abs($bigger)) * 100,
...
function listsTranslations()
{
    ...
    Config::set('tobuli.stops_seconds', [
        ...
        '2700' => '> 45 '.trans('front.minute_short'),
    ...
    Config::set('tobuli.listview_fields_trans', [
        ...
        'device_type_id' => trans('validation.attributes.device_type_id'),
...
function getJsConfig()
{
    ...
    return [
            ...
            'channels' => [
                'userChannel' => $user->getSocketChannel(),
            ],
            'settings' => [
                'units' => [
                    'speed'    => [
                ...
                'toggleSidebar' => false,
                'mapShowDeviceSpeed' => settings('main_settings.map_show_device_speed'),
                'aircraftEnabled' => $user->able('flights_info'),
                ...
                'showAircraft' => Arr::get($user->map_controls, 'm_aircraft', $mapControls['showAircraft'] ?? null),
                ...
                'keys' => [
                    ...
                    'here_political_view' => settings('main_settings.here_political_view'),
            ...
            'urls' => [
                ...
                'deviceEdit' => route('devices.edit').'/',
                'deviceSendCommand' => route('send_command.create'),
                ...
                'deviceWidgetUploadImage' => route('device.image.store').'/',
